...
export class RewardClaimService {
  ...
  async getUserClaims(userId: string): Promise<RewardClaimResponseDto[]> {
    const claims: RewardClaimModel[] =
      await this.rewardClaimRepo.findAllByUserId(userId);
    return claims.map(this.toResponseDto);
  }

  async getAllClaims(eventId?: string): Promise<RewardClaimResponseDto[]> {
    const claims: RewardClaimModel[] = eventId
      ? await this.rewardClaimRepo.findAllByEventId(eventId)
      : await this.rewardClaimRepo.findAll();

    return claims.map(this.toResponseDto);
  }

  private toResponseDto(claim: RewardClaimModel): RewardClaimResponseDto {
    return {
      id: claim.id,
